...
@app.callback(
    [Output("user-apk-results", "children"),
     ...
     Output("user-apk-csv-store-urls",       "data"),
     Output("user-apk-csv-store-subdomains", "data"),
     Output("user-apk-csv-store-domains",    "data"),
     Output("user-apk-btn-csv-urls",         "disabled"),
     Output("user-apk-btn-csv-subdomains",   "disabled"),
     Output("user-apk-btn-csv-domains",      "disabled")],
...
def process_apks_callback(n_clicks, stored_data, highlight_config, num_cores, parser_selection, sort_order):
    ...
    try:
        ...
        if not has_capacity():
            ...
            return [], "", {"display": "none"}, False, False, None, {}, session_id, status_message, status_color, spinner_style, None, None, None, True, True, True
        ...
        # Extract csv_export dicts from results for each data type
        def get_csv(res, dt):
            """Safely extract csv_export dict for a given data type."""
            if not res:
                return None
            fig_result = res.get(dt)
            if not fig_result:
                return None
        ...
        return output_results, "", {"display": "none"}, False, False, None, feature_info_store, session_id, status_message, status_color, spinner_style, csv_urls, csv_subdomains, csv_domains, csv_urls is None, csv_subdomains is None, csv_domains is None
        ...
        return [], error_message, {"display": "block"}, True, False, None, {}, session_id, status_message, status_color, spinner_style, None, None, None, True, True, True
...
# ---------------------------------------------------------------------------
# CSV Download trigger callbacks — user upload pathway
# ---------------------------------------------------------------------------

@app.callback(
    Output('user-apk-download-csv-urls', 'data'),
    Input('user-apk-btn-csv-urls',       'n_clicks'),
    State('user-apk-csv-store-urls',     'data'),
    prevent_initial_call=True,
)
def download_csv_urls_user_apk(n_clicks, csv_store):
...
@app.callback(
    Output('user-apk-download-csv-subdomains', 'data'),
    Input('user-apk-btn-csv-subdomains',       'n_clicks'),
    State('user-apk-csv-store-subdomains',     'data'),
    prevent_initial_call=True,
)
def download_csv_subdomains_user_apk(n_clicks, csv_store):
    if not n_clicks or not csv_store:
        raise PreventUpdate
    raw_b64 = csv_store['content'].split(',', 1)[1]
    return {'content': raw_b64, 'filename': csv_store['filename'],
...
@app.callback(
    Output('user-apk-download-csv-domains', 'data'),
    Input('user-apk-btn-csv-domains',       'n_clicks'),
    State('user-apk-csv-store-domains',     'data'),
    prevent_initial_call=True,
)
def download_csv_domains_user_apk(n_clicks, csv_store):
    if not n_clicks or not csv_store:
        raise PreventUpdate
    raw_b64 = csv_store['content'].split(',', 1)[1]
    return {'content': raw_b64, 'filename': csv_store['filename'],
