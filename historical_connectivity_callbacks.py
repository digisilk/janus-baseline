...
@app.callback(
    [Output("results-historical-connectivity", "children"),
     ...
     Output("historical-csv-store-urls",       "data"),
     Output("historical-csv-store-subdomains", "data"),
     Output("historical-csv-store-domains",    "data"),
     Output("historical-btn-csv-urls",         "disabled"),
     Output("historical-btn-csv-subdomains",   "disabled"),
     Output("historical-btn-csv-domains",      "disabled")],
...
def process_apks_callback(n_clicks, api_key, start_date, end_date, package_list_input, desired_versions, highlight_config, num_cores, parser_selection):
    ...
    if not package_list_input:
        return [], "Please select an app to analyse", {"display": "block"}, True, False, "Analyse App", None, None, "Waiting for app selection...", "warning", {"display": "none"}, None, None, None, True, True, True
    ...
    if cfg.get('show_api_key_input', True):  # Only check if API key input is visible
        if not api_key or len(api_key.strip()) == 0:
            return [], "AndroZoo API key is required", {"display": "block"}, True, False, "Analyse App", None, None, "Missing API key", "danger", {"display": "none"}, None, None, None, True, True, True
        if len(api_key.strip()) != 64 or not all(c in '0123456789abcdefABCDEF' for c in api_key.strip()):
            return [], "Invalid AndroZoo API key format", {"display": "block"}, True, False, "Analyse App", None, None, "Invalid API key", "danger", {"display": "none"}, None, None, None, True, True, True
    ...
    try:
        ...
        if not has_capacity():
            ...
            return [], "Server is busy. Please try again later.", {"display": "block"}, True, False, "Analyse App", None, session_id, status_message, status_color, spinner_style, None, None, None, True, True, True
        ...
        def get_csv(results, data_type):
            """Safely extract the csv_export dict for a data type."""
            if not results:
                return None
            fig_result = results.get(data_type)
            if not fig_result:
                return None
            return fig_result.get('csv_export')

        csv_urls       = get_csv(results, 'urls')
        csv_subdomains = get_csv(results, 'subdomains')
        csv_domains    = get_csv(results, 'domains')

        return output_results, "", {"display": "none"}, False, False, "Analyse App", None, session_id, status_message, status_color, spinner_style, csv_urls, csv_subdomains, csv_domains, csv_urls is None, csv_subdomains is None, csv_domains is None
    except Exception as e:
        ...
        return [], error_message, {"display": "block"}, True, False, "Analyse App", None, session_id, status_message, status_color, spinner_style, None, None, None, True, True, True
...
# ---------------------------------------------------------------------------
# CSV Download trigger callbacks — historical pathway
# ---------------------------------------------------------------------------

@app.callback(
    Output('historical-download-csv-urls', 'data'),
    Input('historical-btn-csv-urls',       'n_clicks'),
    State('historical-csv-store-urls',     'data'),
    prevent_initial_call=True,
)
def download_csv_urls_historical(n_clicks, csv_store):
...
@app.callback(
    Output('historical-download-csv-subdomains', 'data'),
    Input('historical-btn-csv-subdomains',       'n_clicks'),
    State('historical-csv-store-subdomains',     'data'),
    prevent_initial_call=True,
)
def download_csv_subdomains_historical(n_clicks, csv_store):
    if not n_clicks or not csv_store:
        raise PreventUpdate
    raw_b64 = csv_store['content'].split(',', 1)[1]
    return {'content': raw_b64, 'filename': csv_store['filename'],
...
@app.callback(
    Output('historical-download-csv-domains', 'data'),
    Input('historical-btn-csv-domains',       'n_clicks'),
    State('historical-csv-store-domains',     'data'),
    prevent_initial_call=True,
)
def download_csv_domains_historical(n_clicks, csv_store):
    if not n_clicks or not csv_store:
        raise PreventUpdate
    raw_b64 = csv_store['content'].split(',', 1)[1]
    return {'content': raw_b64, 'filename': csv_store['filename'],
