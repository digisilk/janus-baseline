...
def generate_csv_export(all_data, package_name, data_type):
    """
    Build a base64-encoded CSV download payload for a single data type
    (urls, subdomains, or domains) from the raw all_data list produced by
    process_uploaded_apks / plot_data.

    The all_data list for the upload pathway uses the key 'Data' (capital D)
    and 'ui_order' rather than 'vtscandate', so this variant handles that
    schema difference from the other two pathways.

    Returns a dict with keys:
        content   – "data:text/csv;base64,<encoded>" href string
        filename  – suggested download filename
        summary   – short human-readable string describing row count

    Returns None if there is no data to export.

    Column schema
    -------------
    package_name | data_type | value | version | vtscandate | count |
    ...
    try:
        ...
        # Upload pathway stores the extracted value in 'Data' (capital D)
        # and scan date in 'vtscandate' (may be today's date placeholder)
        if 'Data' not in df.columns:
            return None

        df_work = df[['version', 'vtscandate', 'Data']].copy()
        df_work.rename(columns={'Data': 'value'}, inplace=True)
        df_work['value'] = df_work['value'].apply(
            lambda x: truncate_string(x, MAX_STRING_LENGTH)
        )
        df_work['count'] = 1
        df_work = (
            df_work
            .groupby(['version', 'vtscandate', 'value'], as_index=False)
            .sum()
        )
        df_work['vtscandate'] = (
            pd.to_datetime(df_work['vtscandate'], errors='coerce')
            .dt.strftime('%Y-%m-%d')
            .fillna(df_work['vtscandate'])
        )
        ...
        return {
            ...
            'filename': filename,
            'summary': f"{len(df_export)} rows · {df_export['version'].nunique()} versions",
        }
...
def generate_csv_export_with_highlights(all_data, package_name, data_type, highlight_config):
    """
    Variant of generate_csv_export that also populates the highlight_match column.

    highlight_config: list of dicts with 'regex' and 'color' keys
                      (the format used throughout the upload pathway)
    """
    ...
    try:
        ...
        def find_match(value):
            for highlight in reversed(highlight_config):
                if re.search(highlight['regex'], str(value), re.IGNORECASE):
                    return f"{highlight['regex']} ({highlight['color']})"
            return ''
