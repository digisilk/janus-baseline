...
def generate_csv_export(all_data, package_name, data_type):
    ...
    process_package_apks / plot_data.
    ...
    try:
        ...
        df_work.rename(columns={data_type: 'value'}, inplace=True)
        ...
        df_work['vtscandate'] = (
            pd.to_datetime(df_work['vtscandate']).dt.strftime('%Y-%m-%d')
        )
        ...
        df_export = df_work[columns].sort_values(
...
def generate_csv_export_with_highlights(all_data, package_name, data_type, highlight_config):
    ...
    highlight_config: dict {regex_pattern: colour_string}  (same format used by plot_data)
    """
